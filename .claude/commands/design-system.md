...
Use this guide when building any new page, component, or feature for The Launchpad platform. Follow these patterns exactly to maintain visual consistency.
...
brand-100: #E4DEFF   (borders, soft bg)
brand-200: #C9BBFF
brand-300: #A897FF
brand-400: #8B72FF   (accent text, labels)
brand-500: #6C47FF   (focus rings)
brand-600: #5B38F5   (primary buttons, active states) ← PRIMARY
brand-700: #4829E0   (hover states)
brand-800: #3520C4
brand-900: #1A1033   (ink / headings) ← TEXT PRIMARY
```
...
### Semantic Colors
```
Success:   #10B981 (emerald-500) — completed states, green borders
Warning:   #F59E0B (amber-500) — badges, gold accents
Error:     #EF4444 (red-500) — danger buttons, error states
Info:      #3B82F6 (blue-500) — retry grants, info badges
```

### CSS Custom Properties (globals.css)
```css
--bg-page:       #F8F6FF        (page background — light lavender)
--bg-hero:       linear-gradient(145deg, #EDEAFF 0%, #F5F3FF 50%, #FAFAFE 100%)
--border-soft:   #E4DEFF        (default card/section borders)
--text-ink:      #1A1033        (primary text)
...
- **Section labels:** `text-[10px] font-bold uppercase tracking-widest text-brand-400` or `text-gray-400`
...
**Learner pages:**
- Navbar fixed top, h-14, `bg-white/90 backdrop-blur-md`, border `#E4DEFF`
- Content: `max-w-7xl mx-auto px-4 pt-20` (accounts for fixed nav)
- Background: `#F8F6FF` (page-bg)

**Admin pages:**
- Fixed sidebar left, w-60, `bg-gray-900`
- Content: `ml-60 flex-1 p-8`
...
- Header: `px-6 py-4 border-b` with title + X close button
- Body: `p-6`

### Notification Bell (`components/ui/NotificationBell.tsx`)
- Bell icon 17px, 8x8 rounded-xl container
- Unread badge: `bg-red-500 text-white text-[9px] font-black` animated with spring
- Dropdown: `w-80 bg-white rounded-2xl` with brand shadow `rgba(91,56,245,0.12)`
- Border: `1px solid #E4DEFF`
- Section labels: `text-[10px] font-bold uppercase tracking-widest text-gray-400`

### XP Bar (`components/ui/XPBar.tsx`)
- Track: `h-2 bg-white/20 rounded-full`
- Fill: gradient `#818cf8 → #6366f1 → #7c3aed`, animated width with spring
- Labels: `text-xs text-gray-400`

### XP Toast (`components/ui/XPGainToast.tsx`)
- Pill: `bg-brand-600 text-white text-sm font-bold px-3.5 py-2 rounded-full`
- Shadow: `shadow-lg shadow-brand-500/30`
- Lightning icon: `text-yellow-300`
- Float-up animation, auto-dismiss after 900ms

### Badge Unlock Modal (`components/ui/BadgeUnlockModal.tsx`)
...
- Sparkle SVG stars around badge
- CTA: `bg-gradient-to-r from-amber-500 to-orange-500 text-white rounded-xl`
- Fires confetti on open

### Level Up Overlay (`components/ui/LevelUpOverlay.tsx`)
- Similar structure to badge modal but larger (`p-10 max-w-sm`)
- Gradient changes per level (1=gray, 2=blue, 3=indigo, 4=purple, 5=pink, 6=amber, 7=yellow)
- Pulsing glow ring behind card
- CTA: `bg-gradient-to-r from-brand-600 to-purple-600`
- Auto-closes after 4 seconds
- Fires "celebration" confetti (double-burst)

### Confetti (`components/ui/ConfettiCannon.tsx`)
- `useConfetti()` hook returns a fire function
- Two intensities: `medium` (60 particles, centered) and `celebration` (2x 80 particles from sides)
- Colors: `['#4f46e5', '#7c3aed', '#06b6d4', '#10b981']` and gold variants
...
### Sidebar Navigation
- Dark sidebar: `bg-gray-900 text-white w-60 fixed`
- Active link: `bg-brand-600 text-white`
- Inactive: `text-gray-300 hover:bg-gray-800 hover:text-white`
- Items: `px-3 py-2.5 rounded-lg text-sm font-medium`
...
- Sections: white card `rounded-xl border border-gray-200 p-5`
- Sub-items: rows with icons, titles, action buttons
- Inline add/delete: trash icon (Trash2 from lucide, red hover), plus button for adding

### CRUD Pattern (Resources/Projects)
...
### Week Cards (`components/learner/WeekCard.tsx`)
...
- Shake animation on click (rejected access)

**Unlocked state:**
- White card, brand border, top accent line (gradient `#5B38F5 → #7C3AED`)
- Hover: lift `-4px`, purple shadow
- Badge icon with hover rotate
- Section label: `text-[10px] font-bold uppercase tracking-widest` in brand-500
- Progress bar: `h-1.5 rounded-full` on `#F0ECFF` track, brand gradient fill
- CTA: "Start mission" / "Continue mission" / "Mission complete"

**Completed state:**
- Green border `#10B981`, green accent line, green text
- CheckCircle2 icon (spring scale-in)

### Inline Content Rows (Week Detail Page)
- **Subtopics:** white rows in topic sections
...
- Border: `1px solid #E4DEFF`
- Logo: rocket emoji + "The Launchpad" in `font-black`
- Right side: streak pill (orange), rankings link, notification bell, avatar + name, logout
- Avatar fallback: initials on `linear-gradient(135deg, #5B38F5, #7C3AED)`
...
### Spring Presets
```ts
spring:       { stiffness: 400, damping: 28 }      // default
smoothSpring: { stiffness: 220, damping: 24 }      // gentle transitions
bouncySpring: { stiffness: 500, damping: 22 }      // celebrations
gentleSpring: { stiffness: 150, damping: 20 }      // slow reveals
```

### Motion Variants
- `fadeUp` — slide from y:16, spring in (primary for list items)
- `fadeIn` — opacity only, 350ms ease-out
- `scaleIn` — scale from 0.7, bouncy spring (badges, icons)
- `scaleInSmooth` — scale from 0.9, smooth spring
- `slideInRight` / `slideInLeft` — horizontal slide with spring
- `popIn` — scale 0 + rotate -12 → 1 + 0 (celebration pop)
- `pulsate` — scale 1↔1.06, opacity 1↔0.85, infinite 2.4s cycle

### Stagger Patterns
- `staggerContainer` — 80ms between children, 50ms initial delay
- `staggerContainerFast` — 50ms between children, no delay

### Hover Effects
- `cardHover` — y:-4, purple-tinted shadow
- `cardHoverSubtle` — y:-2, neutral shadow
- `shakeKeyframes` — x: [0, -8, 8, -6, 6, -4, 4, 0] (locked card rejection)
...
1. **Purple is brand, green is success, orange is projects/warnings** — never mix these semantics
2. **Spring physics everywhere** — no CSS transitions for interactive elements, always Framer Motion springs
3. **Cards have 1.5px borders** — not 1px, not 2px. Brand-100 default, green for completed
4. **Rounded-2xl for cards, rounded-xl for inner elements, rounded-lg for buttons**
5. **Stagger children in lists** — always use staggerContainer for list reveals
6. **Celebration pipeline:** confetti → badge modal → XP toast → level up overlay (if applicable)
7. **10px uppercase tracking-widest** for section/category labels
8. **Never auto-approve on error** — all AI evaluation defaults to `needs_revision`
9. **Three project scopes:** subtopic (legacy), topic (multiple), week (capstone) — render differently by scope
10. **Admin is gray-900 sidebar + gray-50 content; Learner is white nav + lavender (#F8F6FF) content**
